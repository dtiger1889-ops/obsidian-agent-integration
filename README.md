...
land in a task index that lives in the vault itself. Modeled on the
"LLM-maintained wiki" idea, adapted to PARA.
...
| [extract_approvals.py](operations/extract_approvals.py) / [extract_approvals.ps1](operations/extract_approvals.ps1) | Cross-platform Python and Windows PowerShell versions of the ledger script: scrape proposal-vs-actual destinations into an append-only JSON record |
...
- **Never add a field a human has to hand-populate.** The one field that needed
  manual entry was blank on nearly every item, so it was removed.
- **Use agent-stamped tiers, not human-maintained priority.** `now`, `soon`, and
  `someday` keep speculative work out of the default queue without deleting it.
- **Group the agent's plate by context needed to finish.** Small, medium, and
  large are routing hints for the next agent session, not promises about time.
