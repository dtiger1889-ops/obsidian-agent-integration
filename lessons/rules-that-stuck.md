...
**Cost:** the priority field was blank on nearly every item because it required
a human to make a comparative judgment at capture time. It was removed. "Who
can even do this" is answerable instantly and never decays.
...
**Cost:** see the retired priority field above. This is the single most reliable
predictor of whether a tracking system survives contact with real life.

### ★ Let the agent stamp tiers; group its plate by context

`tier: now|soon|someday` keeps speculative work out of default views without
deleting it, while `est_context: small|medium|large` tells the next agent how
much orientation the item needs. Neither asks the human to maintain metadata.
The agent rechecks stale items for evidence that they became pointless; it does
not silently demote or close them.

**Cost:** a flat project-grouped queue became overwhelming, and a manual
priority field carried no signal. The useful questions were "should this be in
the active surface at all?" and "what size session can finish it?"
