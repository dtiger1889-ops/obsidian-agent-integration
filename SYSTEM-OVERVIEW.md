...
You write a note on your phone; an AI agent does the librarian work — filing the
...
- **Agent projects** (`<WORKSPACE>/<project>/`) are where *work* happens —
...
so the agent's search tools work on it normally. That accident of placement is
...
a local agent runtime with file access. What was missing was the maintenance
loop (the librarian) and the bridge that turns captured to-dos into tracked action items —
...
| Layer | Owner | Rule |
|---|---|---|
| Your raw notes (inbox captures, journal) | You | The agent reads, never rewrites |
| The organized vault (PARA files, links, tags) | The agent maintains, with your approval | Filed with dated headings, wikilinks, schema tags |
| The rulebook (vault `HOME.md`, tag schema, specs) | You | The agent follows it, never edits it without you |
...
1. **You capture on the phone.** Any thought → a note in `00 Inbox/`. Offline,
   ...
   the other way round. A fail-closed lifecycle then separates unreviewed clips,
   reviewed keepers, archive candidates, and delete candidates without adding a
   nested archive — see
   [web-clippings-lifecycle.md](lessons/web-clippings-lifecycle.md).)*
2. **You (or a schedule, later) say "process my inbox."** The agent sweeps the
...
## Chunk 7 — The /obsidian skill: one door, five rooms

Everything user-facing is one skill:

- **Query** (default, read-only): "what was that reference detail?" from any project —
  The agent orients via `HOME.md`, finds the one note, answers with the path.
...
- **Sprint-notes** (added later): you type instructions into the `note` column
  ...
  notes" and hand over the whole batch. The agent answers in a `reply` column and
...
without maintaining two behavioral copies —
...
typed-capture folder and the web-clipper folder · five Bases (task index, to-do
...
**Measured, not assumed:** the staging gate records what the agent proposed *and*
