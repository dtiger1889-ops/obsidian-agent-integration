...
foreach ($f in $files) {
    ...
    # Prefer title + created, then recover a renamed note by its unique created
    # stamp. A move keeps the same id; a rename updates the title without
    # duplicating the proposal-versus-outcome record.
    $title = [IO.Path]::GetFileNameWithoutExtension($f.Name)
    $createdStamp = $fm['created']
    $preferredId = '{0}|{1}' -f $title, $createdStamp
    $createdMatches = @($ledger.Keys | Where-Object {
        $createdStamp -and $ledger[$_].created -eq $createdStamp
    })
    $id = if ($ledger.ContainsKey($preferredId)) {
        $preferredId
    } elseif ($createdMatches.Count -eq 1) {
        $createdMatches[0]
    } else {
        $preferredId
    }
    ...
    if ($ledger.ContainsKey($id)) {
        ...
        $rec = [pscustomobject]@{ id = $id; first_seen = $Today }
        $ledger[$id] = $rec
    }
    $rec | Add-Member -NotePropertyName 'title' -NotePropertyValue $title -Force
...
foreach ($id in @($ledger.Keys)) {
    if (-not $seenIds.Contains($id)) {
        ...
        $ledger[$id] | Add-Member -NotePropertyName 'vanished' -NotePropertyValue $false -Force
    }
}
